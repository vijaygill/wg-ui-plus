...
def are_network_addresses(value, throw_exception=True):
    ...
    if value:
        ...
        parts_result = [
            (x, is_network_address(x, throw_exception=False)) for x in parts
        ]
...
class PeerGroup(models.Model):
    ...
    peers = models.ManyToManyField("PeerGroupPeer", blank=True)
    targets = models.ManyToManyField("PeerGroupTarget", blank=True)
...
class Peer(models.Model):
    ...
    def delete(self):
        print(f">>>>>> {self}")
        pgs = PeerGroup.objects.all()
        for pg in pgs:
            if self in pg.peers.all():
                print(f">>>>>> found {self} in {pg}. Removing...")
                pg.peers.remove(self)
        for pg in self.peer_groups.all():
            print(f">>>>>> found {pg} in {self}. Removing...")
            pg.peers.remove(self)
            self.peer_groups.remove(pg)
        return super().delete()
...
class Target(models.Model):
    ...
    peer_groups = models.ManyToManyField(
        "PeerGroup",
        blank=True,
        through='PeerGroupTarget',
    )

    def __str__(self):
        return f"{self.name} - {self.description}"

class PeerGroupPeer(models.Model):
    peer = models.ForeignKey(Peer, on_delete=models.CASCADE)
    peer_group = models.ForeignKey(PeerGroup, on_delete=models.CASCADE)

class PeerGroupTarget(models.Model):
...
class ServerConfiguration(models.Model):
    ...
    upstream_dns_ip_address = models.CharField(
        max_length=255, null=False, blank=False, validators=[is_single_address]
    )
    local_networks = models.CharField(
        max_length=512, null=True, blank=True, validators=[are_network_addresses]
    )
