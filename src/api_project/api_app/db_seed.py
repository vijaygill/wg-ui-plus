...
SAMPLE_PEERS = [ ('Mobile phone', 'Your mobile phone' ), ('Laptop', 'May need access to VPN occasionally')]
...
def populate_dictionary_data(apps, schema_editor):
    ...
    for peer_name, description in SAMPLE_PEERS:
        ...
        sc_intf = ipaddress.ip_interface(serverConfiguration.network_address)
        ...
        public_key, private_key = generate_keys()
        peer = Peer(name = peer_name, description = description,
                    public_key = public_key, private_key = private_key,
